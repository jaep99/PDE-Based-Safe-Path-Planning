function PDE_SafePathImplementation()
% PDE_SafePathImplementation (Eikonal / Fast Marching)
%
% Solve the Eikonal PDE  ||grad T|| = c(x)  with a "soft" cost near obstacles,
% then do a backtrace from goal to start to get a global path.
% 
% This version fixes the "Inputs A and B must have the same number of columns"
% error by initializing pq.data = zeros(0,2).
%
% Steps:
%   1) environment.m => obsMap(1=obs,0=free), startPos, goalPos, xvec,yvec,nx,ny
%   2) We define cMap = 1 + alpha e^{-beta dist}, obstacle=1e5
%   3) Solve eikonal T (start => 0) via a simple "fast marching" style
%   4) Backtrace from goal => start
%   5) Plot final path
...
for rv = reqVars
    if ~ismember(rv{1}, envVars)
        error(['Variable ',rv{1},' not found in base workspace. Run environment.m first.']);
    end
end
...
cMap(obsMap==1) = 1e5;  % obstacle => infinite cost
...
% priority queue as struct: 
pq.data = zeros(0,2);   % Nx2
pq.keys = [];           % Nx1
...
% If item in pq.data => update key if smaller
% else push new
